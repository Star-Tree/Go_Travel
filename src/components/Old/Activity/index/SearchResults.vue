<script setup>
    ...
    const storeOneCity = computed(() => store.getters["Activity/oneCity"]);
    const storeSearchText = computed(() => store.getters["Activity/searchText"]);

    const storeSelectRelevantOrNearby = computed(() => {
        return store.getters["Activity/selectRelevantOrNearby"];
    });

    const storeDateTime = computed(() => store.getters["Activity/dateTime"]);
    const storeCategory = computed(() => store.getters["Activity/category"]);
    const storeResetFilter = computed(() => store.getters["Activity/resetFilter"]);

    const storeShowSearchResultArr = computed(() => {
        return store.getters["Activity/showSearchResultArr"];
    });
    ...
    async function changeSelectOption(newOption) {
        try {
            if (newOption === "nearby") {
                ...
                await store.dispatch("Activity/handChangeSelectRelevantOrNearby", "nearby");

                // 切換成 附近結果的 所有資料。
                await store.dispatch("Activity/handNearbyOfSearchResultArrInit", coordsObj);
            }

            else if (newOption === "relevant") {
                // 一切換到 relevant，就也同步改變 store中的 option
                await store.dispatch("Activity/handChangeSelectRelevantOrNearby", "relevant");

                // 切換成 相關結果的 所有資料。
                await store.dispatch("Activity/handShowSearchResultArrInit", activityObj);
            }
        }
    ...
    async function changeDate(selectDate) {
        try {
            ...
            if (!isNaN(date) && selectRelevantOrNearby.value === "relevant") {
                await store.dispatch("Activity/handShowSearchResultArrInit", activityObj);
                await store.dispatch("Activity/handSelectDate", date);
            ...
            await store.dispatch("Activity/handDateTimeInit", selectDate);
        }
        ...
        try {
            if (e === "recover") {
                ...
                if (categoryObj.category !== "") {
                    await store.dispatch("Activity/handCategoryOfSearchResultArrInit", categoryObj);
            ...
            // 若按下 已篩選過的同顆按鈕，就代表要看 按下 按鈕前的 所有暫存資料。
                ...
                // 一旦取消類別，且 時間日期也清除，就解開 選擇相關搜尋 或 附近搜尋。
                if (dateTime.value === "") {
                    elSelect.value.disabled = false;
                }
            ...
            await store.dispatch("Activity/handCategoryInit", categoryObj.category);
        }
    ...
    watch(storeOneCity, (storeOneCity) => {
        activityObj.oneCity = storeOneCity;
    });

    watch(storeSearchText, (storeSearchText) => {
        activityObj.searchText = storeSearchText;
    });
        ...
        <div class="searchResults_filter">
            <div class="searchResults_filter_header">
                ...
                <div class="filterBlockMobile">
                    <select v-model="selectRelevantOrNearby" ref="elSelect" class="orderMobile">
                        ...
                        <option value="nearby">顯示附近活動</option>
                    </select>
            ...
            <form action="#" method="post" :class="{open: isOpenScrollFilter}">
                ...
                <div class="category" ref="elCategory" @click="filterClass($event)">
                    ...
                    <input type="button" data-category="年度活動" value="年度活動">

                    <input type="button" data-category="節慶活動" value="節慶活動">

                    <input type="button" data-category="藝文活動" value="藝文活動">

                    <input type="button" data-category="親近山海" value="親近山海">
                </div>
            </form>
        </div>

        <ul class="searchResults_show" v-show="!isOpenForNoResult">
            <li v-for="(item) in storeShowSearchResultArr" :key="item.ActivityID">
