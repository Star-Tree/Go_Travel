<script setup>
    ...
    const storeAllCityArr = computed(() => store.getters["Activity/allCityArr"]);
    ...
    const handSearch = () => {
        ...
        activityObj.searchText = searchText.value;
        activityObj.oneCity = oneCity.value;

        store.dispatch("Activity/handShowSearchResultArrInit", activityObj);
        store.dispatch("Activity/handChangeSelectRelevantOrNearby", "relevant");
        store.dispatch("Activity/handDateTimeInit", "");
        store.dispatch("Activity/handCategoryInit", "");
        store.dispatch("Activity/handResetFilterInit");
    };

    watch(searchText, (searchText) => {
        // 同步改變 store 的 搜尋文字
        store.dispatch("Activity/handSearchTextInit", searchText);
    });

    watch(oneCity, (oneCity) => {
        // 同步改變 store 的 城市文字。
        store.dispatch("Activity/handOneCityInit", oneCity);
    });

    const init = async () => {
        // 指派 select-option 放入 台灣各縣市
        await store.dispatch("Activity/handAllCityArrInit");
...
<template>
    <div class="searchBar">
        <span class="searchBar_pageTipsMobile">活動頁 / 搜尋結果</span>
