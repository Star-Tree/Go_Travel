<script setup>
    ...
    const storeShowDynamicPageArr = computed(() => {
        return store.getters["Activity/showDynamicPageArr"];
    });

    const storeNearbyOfShowDynamicPageArr = computed(() => {
        if (deviceWidth.value <= 1024 && deviceWidth.value > 767) {  // 平板就只給2個地點
            return store.getters["Activity/nearbyOfShowDynamicPageArr"].filter((item, index) => {
                return index <= 1;
            });
        } else {
            return store.getters["Activity/nearbyOfShowDynamicPageArr"];
        }
    });
    ...
    const init = () => {
        ...
        coordsObj.currentId = storeShowDynamicPageArr.value.ActivityID;

        store.dispatch("Activity/handNearbyOfShowDynamicPageArrInit", coordsObj);
    };
