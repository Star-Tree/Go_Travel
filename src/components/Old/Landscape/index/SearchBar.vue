<script setup>
    ...
    const storeAllCityArr = computed(() => store.getters["Landscape/allCityArr"]);
    ...
    const handSearch = () => {
        ...
        landscapeObj.oneCity = oneCity.value;

        store.dispatch("Landscape/handShowSearchResultArrInit", landscapeObj);
        store.dispatch("Landscape/handChangeSelectRelevantOrNearby", "relevant");
        store.dispatch("Landscape/handCheckboxTicketInit", false);
        store.dispatch("Landscape/handCategoryInit", "");
        store.dispatch("Landscape/handResetFilterInit");
    };

    watch(searchText, (searchText) => {
        // 同步改變 store 的 搜尋文字(首頁-banner & 景點頁-search)
        store.dispatch("Landscape/handSearchTextInit", searchText);
    });

    watch(oneCity, (oneCity) => {
        // 同步改變 store 的 城市文字。
        store.dispatch("Landscape/handOneCityInit", oneCity);
    });

    const init = () => {
        // 指派 select-option 放入 台灣各縣市
        store.dispatch("Landscape/handAllCityArrInit");
...
<template>
    <div class="searchBar">
        <span class="searchBar_pageTipsMobile">景點頁 / 搜尋結果</span>
