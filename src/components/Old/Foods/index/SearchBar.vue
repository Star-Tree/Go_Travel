<script setup>
    ...
    const storeAllCityArr = computed(() => store.getters["Foods/allCityArr"]);
    ...
    const handSearch = () => {
        ...
        foodsObj.searchText = searchText.value;
        foodsObj.oneCity = oneCity.value;

        store.dispatch("Foods/handShowSearchResultArrInit", foodsObj);
        store.dispatch("Foods/handChangeSelectRelevantOrNearby", "relevant");
        store.dispatch("Foods/handCategoryInit", "");
        store.dispatch("Foods/handResetFilterInit");
    };

    watch(searchText, (searchText) => {
        // 同步改變 store 的 搜尋文字
        store.dispatch("Foods/handSearchTextInit", searchText);
    });

    watch(oneCity, (oneCity) => {
        // 同步改變 store 的 城市文字。
        store.dispatch("Foods/handOneCityInit", oneCity);
    });

    const init = async () => {
        // 指派 select-option 放入 台灣各縣市
        await store.dispatch("Foods/handAllCityArrInit");
...
<template>
    <div class="searchBar">
        <span class="searchBar_pageTipsMobile">美食頁 / 搜尋結果</span>
