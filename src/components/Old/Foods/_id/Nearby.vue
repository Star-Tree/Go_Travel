<script setup>
    ...
    const storeNearbyOfShowDynamicPageArr = computed(() => {
        if (deviceWidth.value <= 1024 && deviceWidth.value > 767) {  // 平板就只給2個地點
            return store.getters["Foods/nearbyOfShowDynamicPageArr"].filter((item, index) => {
                return index <= 1;
            });
        } else {
            return store.getters["Foods/nearbyOfShowDynamicPageArr"];
        }
    });
    ...
    const init = () => {
        ...
        coordsObj.currentId = storeShowDynamicPageArr.value.RestaurantID;

        store.dispatch("Foods/handNearbyOfShowDynamicPageArrInit", coordsObj);
    };
