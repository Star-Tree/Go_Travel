<script setup>
    ...
    const storeSelectRelevantOrNearby = computed(() => {
        return store.getters["Hotel/selectRelevantOrNearby"];
    });

    const storeCategory = computed(() => store.getters["Hotel/category"]);
    const storeResetFilter = computed(() => store.getters["Hotel/resetFilter"]);

    const storeShowSearchResultArr = computed(() => {
        return store.getters["Hotel/showSearchResultArr"];
    });
    ...
    async function changeSelectOption(newOption) {
        try {
            if (newOption === "nearby") {
                ...
                await store.dispatch("Hotel/handChangeSelectRelevantOrNearby", "nearby");

                // 切換成 附近結果的 所有資料。
                await store.dispatch("Hotel/handNearbyOfSearchResultArrInit", coordsObj);
            }

            else if (newOption === "relevant") {
                // 一切換到 relevant，就也同步改變 store中的 option
                await store.dispatch("Hotel/handChangeSelectRelevantOrNearby", "relevant");

                // 切換成 相關結果的 所有資料。
                await store.dispatch("Hotel/handShowSearchResultArrInit", hotelObj);
            }
        }
        ...
        try {
            if (e === "recover") {
                ...
                if (categoryObj.category !== "") {
                    await store.dispatch("Hotel/handCategoryOfSearchResultArrInit", categoryObj);
            ...
            await store.dispatch("Hotel/handCategoryInit", categoryObj.category);
        }
    ...
    watch(storeOneCity, (storeOneCity) => {
        hotelObj.oneCity = storeOneCity;
    });

    watch(storeSearchText, (storeSearchText) => {
        hotelObj.searchText = storeSearchText;
    });
        ...
        <div class="searchResults_filter">
            <div class="searchResults_filter_header">
                ...
                <div class="filterBlockMobile">
                    <select v-model="selectRelevantOrNearby" ref="elSelect" class="orderMobile">
                        ...
                        <option value="nearby">顯示附近旅館</option>
                    </select>
            ...
            <form action="#" method="post" :class="{open: isOpenScrollFilter}">
                ...
                <div class="category" ref="elCategory" @click="filterClass($event)">
                    ...
                    <input type="button" data-category="一般旅館" value="一般旅館">

                    <input type="button" data-category="一般觀光旅館" value="觀光旅館">

                    <input type="button" data-category="國際觀光旅館" value="國際旅館">

                    <input type="button" data-category="民宿" value="民宿">
                </div>
            </form>
        </div>

        <ul class="searchResults_show" v-show="!isOpenForNoResult">
            <li v-for="(item) in storeShowSearchResultArr" :key="item.HotelID">
                <router-link :to="`/hotel/${ item.HotelID }`">
