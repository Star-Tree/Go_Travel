import { dataAllCity,
         apiGetSpecifyOfHotel,
         apiGetNearbyOfHotel,
         apiGetOneLocationOfHotel,
...
export default {
  ...
  async handShowSearchResultArrInit({commit}, {searchText = "", oneCity = ""}) {
    try {
      const res = await apiGetSpecifyOfHotel(searchText, oneCity);
      ...
      const searchResultArr = res.data.map(item => {
        ...
        if (!item.Picture.PictureUrl1) {
          item.Picture.PictureUrl1 = "https://i.imgur.com/eFtqHKt.png";
  ...
  async handNearbyOfSearchResultArrInit({commit}, coordsObj) {
    try {
      const res = await apiGetNearbyOfHotel(coordsObj, 3000);
  ...
  async handShowDynamicPageArrInit({commit}, HotelID) {
    try {
      const res = await apiGetOneLocationOfHotel(HotelID);
      ...
      if (!dynamicPageArr.Spec) {
        dynamicPageArr.Spec = "無";
      } else {
        dynamicPageArr.Spec = dynamicPageArr.Spec.replaceAll(";", "<br / >");
      }

      if (!dynamicPageArr.ServiceInfo) {
        dynamicPageArr.ServiceInfo = "無";
      } else {
        dynamicPageArr.ServiceInfo = dynamicPageArr.ServiceInfo.replaceAll(",", "<br / >");
      }

      if (!dynamicPageArr.ParkingInfo) {
        dynamicPageArr.ParkingInfo = "無";
      } else {
        dynamicPageArr.ParkingInfo = dynamicPageArr.ParkingInfo.replaceAll(":", ": ");
      } 

      if (!dynamicPageArr.Picture.PictureUrl1) {
        dynamicPageArr.Picture.PictureUrl1 = "https://i.imgur.com/efVjmp9.png";
      }
  ...
  async handNearbyOfShowDynamicPageArrInit({commit}, coordsObj) {
    try {  // 取得動態分頁位置的 附近0.5公里內的其它位置項目。
      const res = await apiGetNearbyOfHotel(coordsObj, 500);
      ...
      const tempArr = res.data.filter((item, index) => {
        ...
        return (item.HotelID !== coordsObj.currentId) && index < 4;
      });
