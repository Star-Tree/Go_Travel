import { dataAllCity,
         apiGetSpecifyOfLandscape,
         apiGetNearbyOfLandscape,
         apiGetOneLocationOfLandscape,
...
export default {
  ...
  handCheckboxTicketInit({commit}, checkboxTicket) {
    commit("checkboxTicketInit", checkboxTicket);
  },
  ...
  async handShowSearchResultArrInit({commit}, {searchText = "", oneCity = ""}) {
    try {
      const res = await apiGetSpecifyOfLandscape(searchText, oneCity);
      ...
      const searchResultArr = res.data.map(item => {
        ...
        if (!item.Picture.PictureUrl1) {
          item.Picture.PictureUrl1 = "https://i.imgur.com/aCp8ZhT.png";
  ...
  handCheckTicketInfo({commit}) {
    commit("checkTicketInfo");
  },

  async handNearbyOfSearchResultArrInit({commit}, coordsObj) {
    try {
      const res = await apiGetNearbyOfLandscape(coordsObj, 3000);
  ...
  async handShowDynamicPageArrInit({commit}, scenicSpotID) {
    try {
      const res = await apiGetOneLocationOfLandscape(scenicSpotID);
      ...
      if (!dynamicPageArr.Picture.PictureUrl1) {
        dynamicPageArr.Picture.PictureUrl1 = "https://i.imgur.com/dMc0pZ3.png";
      }
  ...
  async handNearbyOfShowDynamicPageArrInit({commit}, coordsObj) {
    try {  // 取得動態分頁位置的 附近1公里內的其他位置項目。
      const res = await apiGetNearbyOfLandscape(coordsObj, 1000);
      ...
      const tempArr = res.data.filter((item, index) => {
        ...
        return (item.ScenicSpotID !== coordsObj.currentId) && index < 4;
      });
