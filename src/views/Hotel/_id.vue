<script setup>
    ...
    onMounted(() => {
        ...
        store.dispatch("Hotel/handShowDynamicPageArrInit", route.params.HotelID);
    ...
    watch(() => route.params.HotelID, (HotelID) => {
        if (HotelID) {  // 回到 index.vue 就不要執行，否則會報錯。
            store.dispatch("Hotel/handShowDynamicPageArrInit", HotelID);
        }
    });
</script>
